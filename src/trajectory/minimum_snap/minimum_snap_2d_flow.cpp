#include "minimum_snap_planner/trajectory/minimum_snap/minimum_snap_2d_flow.h"
...
MatXd MinimumSnap2DFlow::PickWaypoints(const nav_msgs::PathPtr &path_ptr) const {
    if (!path_ptr || path_ptr->poses.size() < 2u) {
        throw std::runtime_error("[MinimumSnap2DFlow] Path must contain at least start and goal poses.");
    }
