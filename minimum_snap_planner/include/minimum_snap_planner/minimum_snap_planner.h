...
private:
    ...
    /**
     * @brief Determine if the incoming goal differs from the previous navigation target
     */
    bool isNewGoal(const geometry_msgs::PoseStamped& goal) const;

    /**
     * @brief Clear the currently active waypoint list after a plan has been generated
     */
    void clearActiveWaypoints();
