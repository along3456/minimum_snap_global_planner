...
MatXd MinimumSnap::SolveQPClosedForm(const VecXd &waypoints_pos, const VecXd &waypoints_vel,
    ...
    if (waypoints_pos.size() != waypoints_vel.size()) {
        throw std::invalid_argument("[MinimumSnap] Position and velocity waypoint vectors must have the same length.");
    }
    if (waypoints_vel.size() != waypoints_acc.size()) {
        throw std::invalid_argument("[MinimumSnap] Velocity and acceleration waypoint vectors must have the same length.");
    }
    if (waypoints_pos.size() != segments_time.size() + 1u) {
        throw std::invalid_argument("[MinimumSnap] Number of positions must equal segments_time + 1.");
    }
